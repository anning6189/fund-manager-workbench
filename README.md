...
> 部署边界：本地项目、GitHub 仓库和 `47.95.254.215:8765` 属于开发/迭代线；`xiaofei.becoming.fund` 是独立交付线，已从本地/GitHub/老公网切割，不再跟随本项目后续改动自动更新。详见 `docs/deploy/deployment-boundary.md`。
...
- 开发线新增量化研究框架，支持因子快照、IC/Rank IC、Q1-Q5 分组回测、滚动窗口验证和组合优化输入。
...
- 支持点击持仓股票查看单股走势；
- 展示本周策略与相比上一版的优化；
- 接入站长内部大模型后，自动生成“AI自主投研建议”：包括市场判断、看多/回避行业、个股增配/降配/否决建议、因子权重微调建议；
- 对大模型建议做自动风控校验：限制单票调整幅度、检查信心分、过滤不在当前组合内的股票，并记录采纳/拒绝原因；
- 生成影子组合，用于观察大模型建议是否优于当前正式规则，避免未经验证的建议直接污染正式净值；
- 支持历史组合复盘。
...
AI基金经理使用站长在服务器配置的内部模型 Key；AI研究员问答使用用户自己的 Key。两套模型通道隔离。未配置内部模型时，AI基金经理仍可按规则模型独立运行；配置内部模型后，会增强策略说明、自主投研建议、事件归因和规则自校对能力。

### 9. 量化研究

量化研究页用于把当前选股规则扩展成更标准的“因子研究 + 组合构建 + 策略评估”框架：

- 将现有消费行研评分拆解为 6 个可检验因子：综合 Alpha、质量、估值、催化、风险控制、市场适配；
- 每日固化因子快照，避免后续回看时使用未来数据；
- 回填 T+1、T+5、T+20、T+60 未来收益，其中 T+20/T+60 是中期/中长期核心观察周期；
- 计算 IC、Rank IC、IC_IR、正 IC 占比和样本数，用于检验因子排序能力；
- 按 Q1-Q5 分组回测，观察高分组是否稳定跑赢低分组；
- 提供 20D、60D、120D 滚动窗口验证，识别因子衰减、反向和样本不足；
- 为 AI基金经理组合提供因子有效性加权输入，组合优化继续受单票权重、子行业、换手率和交易成本约束。
...
| `consumer-research-quant-research.timer` | 交易日 17:30 | 生成量化因子快照、未来收益、IC/Rank IC、分组回测和滚动验证 |
...
tools/
  ...
  Compute-QuantResearch.py                  # 量化研究计算入口
  quant_research.py                         # 因子快照、IC/Rank IC、分组回测、滚动验证
