...
class WorkbenchService:
    ...
    def _build_ask_messages(self, question: str, history: list) -> tuple[list, int]:
        ...
        context_lines = [
            ...
            "【研究底座 · 当日内容】",
            "【评分口径】旧的“当日动量40%/估值30%/事件30%”已废弃。当前为 V2 中长期量化框架："
            "质量、风险控制、中长期稳定性是核心 Alpha；估值只作为买点与安全边际约束；催化和市场适配只做小幅辅助修正。"
            "每日主推≤5只，必须来自“可以考虑买入”候选且满足组合层约束；可以考虑买入保留25只；等待买点、长期观察、暂不推荐/行业扫描仅用于分层跟踪和后验校验。"
            "规则审计主看 T+20/T+60，并通过 IC/Rank IC、行业中性 IC、分组回测、滚动样本外验证、交易成本敏感性和组合风险归因检验。",
        ]
        used = 0
